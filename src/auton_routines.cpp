...
void auton_60s_skills_1() {
  ...
  outakeFor(127, 200);
  pros::delay(600);
  // chassis.moveToPoint(0, -8, 4000, {.forwards = false, .maxSpeed = mid_speed,
  // .minSpeed = 50, .earlyExitRange = 1});
  chassis.moveToPoint(0, -18, 4000,
                      {.forwards = false, .maxSpeed = mid_speed - 15});
  chassis.waitUntilDone();
  pros::delay(500);
  ...
  chassis.turnToHeading(0, 2000, {.maxSpeed = max_speed1});
  chassis.waitUntilDone();
  chassis.setPose(findDistToWall(1) - 72, findDistToWall(2) - 72,
                  findHeading(1, 0.0));
  currentPose = chassis.getPose();
  ...
  // max_speed}); //Move towards first ring chassis.waitUntil(10);
  // pros::delay(350);
  spinIntake(127);
  // intake.move(0); //stop intake to avoid outaking preload
  pros::Task lift_up1(
      [=] { // raise lift (later lift may get in way near the wall)
        //////moveLiftToPos(300);
      });
  ...
  chassis.moveToPoint(-24, -26, 2000,
                      {.maxSpeed = mid_speed}); // grabs first ring
  chassis.waitUntilDone();
  pros::delay(100);
  ...
  chassis.moveToPoint(
      -48, -28, 2000,
      {.forwards = false, .maxSpeed = mid_speed}); // backs up from second ring
  pros::delay(250);
  chassis.turnToHeading(180, 2000, {.maxSpeed = max_speed1});
  chassis.waitUntilDone();
  ...
  chassis.moveToPoint(
      -38, -50, 2000,
      {.forwards = false,
       .maxSpeed = max_speed}); // backs up before last (basket) ring
  chassis.waitUntilDone();
  pros::delay(50);
  currentPose = chassis.getPose();
  ...
  float myX_skills = fabs(findDistToWall(3) - 24.0) > 10.0 ? currentPose.y :
  findDistToWall(3) - 72;
  ...
  chassis.turnToHeading(0, 1000, {.maxSpeed = max_speed1});
  pros::delay(80);
  chassis.moveToPoint(-63, -65, 1000,
                      {.forwards = false, .maxSpeed = max_speed});
  ...
  chassis.setPose(findDistToWall(1) - 72, findDistToWall(2) - 72,
                  currentPose.theta);
  pros::delay(50);
  // chassis.moveToPoint(-48, -48, 2000, {.forwards = false, .maxSpeed =
  // max_speed}); chassis.waitUntilDone(); chassis.turnToHeading(0, 2000,
  // {.maxSpeed = max_speed1}); chassis.waitUntilDone();
  // pros::delay(100);
  ...
  lemlib::AngularDirection::CW_CLOCKWISE, .maxSpeed = max_speed1 - 10});
  ...
  */
  ...
  chassis.waitUntilDone();
  pros::delay(1000);
  chassis.moveToPoint(-48, -45, 2000,
                      {.forwards = false, .maxSpeed = max_speed + 10});
  ...
  pros::delay(100);
  chassis.turnToHeading(0, 2000,
                        {.maxSpeed = max_speed1, .earlyExitRange = 0.5});
  chassis.waitUntilDone();
  currentPose = chassis.getPose();
  pros::delay(100);
  chassis.setPose(findDistToWall(1) - 74, findDistToWall(2) - 72,
                  findHeading(1, 0.0)); // reset after ring after first pole
  ...
  chassis.waitUntilDone();
  chassis.setPose(
      findDistToWall(2) - 73, findDistToWall(0) - 72,
      findHeading(2, 90.0)); // reset after coming back turn CHECK AT MAINE
  pros::delay(100);
  chassis.moveToPoint(-1.5, -48, 2000, {.maxSpeed = mid_speed});
  chassis.waitUntilDone();
  spinIntake(0);
  currentPose = chassis.getPose();
  chassis.setPose(currentPose.x, currentPose.y,
                  findHeading(0, 90)); // reset angle at red stake
  printPosition((char *)"Alliance Angle reset", true, true);
  chassis.waitUntilDone();
  // chassis.turnToHeading(90, 2000, {.maxSpeed = max_speed1, .earlyExitRange =
  // 0.5}); chassis.waitUntilDone();
  currentPose = chassis.getPose();
  ...
  chassis.moveToPoint(
      0, -48, 2000,
      {.forwards = false, .maxSpeed = max_speed}); // back out of stake
  spinIntake(0);
  chassis.turnToHeading(270, 2000);
  chassis.waitUntilDone();
  currentPose = chassis.getPose();
  chassis.setPose(currentPose.x, findDistToWall(1) - 72, findHeading(1, 270));
  chassis.moveToPoint(28, -48, 2000,
                      {.forwards = false, .maxSpeed = max_speed}); // grrab goal
  ...
  pros::delay(750);
  spinIntake(127);
  chassis.setPose(72 - findDistToWall(2), findDistToWall(1) - 72,
                  findHeading(1, 270));
  ...
  chassis.turnToHeading(135, 2000, {.maxSpeed = max_speed1});
  chassis.waitUntilDone();
  chassis.moveToPoint(40, -48, 2000, {.forwards = false, .maxSpeed =
  max_speed}); chassis.waitUntilDone(); chassis.turnToHeading(90, 2000,
  {.maxSpeed = 65}); chassis.waitUntilDone(); chassis.moveToPoint(62, -48, 2000,
  {.maxSpeed = max_speed}); printPosition((char *)"5 (saved)", true, true);
  chassis.waitUntilDone();
  intake.move(0);
  pros::delay(350);
  chassis.moveToPoint(48, -48, 2000, {.forwards = false, .maxSpeed =
  max_speed}); chassis.waitUntilDone(); printPosition((char *)"pre drop
  backout", true, true); chassis.turnToHeading(-45, 2000, {.maxSpeed =
  max_speed1}); chassis.waitUntilDone(); intake.move(0); //save ring
  chassis.moveToPoint(65, -65, 2000, {.forwards = false, .maxSpeed =
  max_speed}); chassis.waitUntilDone(); printPosition((char *)"drop", true,
  true); openClamp(); chassis.moveToPoint(42, -42, 2000, {.maxSpeed =
  max_speed}); printPosition((char *)"post drop backout", true, true);
  chassis.turnToHeading(0, 2000, {.maxSpeed = max_speed1});
  chassis.waitUntilDone();
...
*/
  /*
    ...
    pros::delay(100);
    chassis.turnToHeading(0, 2000, {.maxSpeed = max_speed1, .earlyExitRange =
  0.5}); chassis.setPose(72 - findDistToWall(0), findDistToWall(2) - 72,
  currentPose.theta); pros::delay(100); printPosition((char *)"final reset",
  true, true); setBasket(false); //open hood
  ...
  /* took out 2nd basket at wall stake for time
    ...
    chassis.waitUntilDone();
    pros::delay(750);
    chassis.moveToPoint(44.0, 0, 2000, {.forwards = false, .maxSpeed =
  max_speed});
    */
  // pros::delay(1000);
 ...
 */
  ...
  chassis.moveToPoint(-20, 36, 3000,
                      {.forwards = false, .maxSpeed = max_speed});
  chassis.waitUntilDone();
  setBasket(true);
  chassis.moveToPoint(-20, 62, 3000,
                      {.forwards = false, .maxSpeed = max_speed});
  chassis.turnToHeading(90, 2000, {.maxSpeed = max_speed1});
  // chassis.moveToPoint(-24, 65, 1000, {.forwards = false, .maxSpeed =
  // max_speed}); chassis.turnToHeading(90, 2000, {.maxSpeed = max_speed1});
  ...
  // lemlib::AngularDirection::CCW_COUNTERCLOCKWISE}); chassis.waitUntil(90);
  // retractSweep();
  // chassis.waitUntilDone();
  ...
  chassis.waitUntilDone();
  pros::delay(100);
  // chassis.setPose(-72 + findDistToWall(3), 71 - findDistToWall(0),
  // findHeading(0, 270));
  ...
  //  chassis.turnToHeading(-45, 2000, {.direction =
  //  AngularDirection::CW_CLOCKWISE, .maxSpeed = max_speed1});

  // chassis.moveToPose(findDistToWall(0) - 21, findDistToWall(3) - 72, 0, 3000,
  // {.forwards = false, .maxSpeed = max_speed});
  ...
  pros::delay(250);
  currentPose = chassis.getPose();
  chassis.setPose(findDistToWall(1) - 71, 72 - findDistToWall(3),
                  currentPose.theta);
  chassis.waitUntilDone();
  ...
  currentPose = chassis.getPose();
  chassis.moveToPoint(0, 50, 2000, {.forwards = false, .maxSpeed = max_speed});
  chassis.getPose();
  ...
  chassis.turnToHeading(-45, 2000,
                        {.maxSpeed = max_speed1, .earlyExitRange = 1});
  chassis.waitUntilDone();
  pros::delay(1500);
  ...
  // chassis.setPose(74.0 - findDistToWall(3), myX2, findHeading(1, 90),
  // currentPose.theta);
  ...
  chassis.waitUntilDone();
  pros::delay(250);
  chassis.setPose(74.0 - findDistToWall(3), 74.0 - findDistToWall(1),
                  findHeading(1, 90));
  pros::c::delay(250);
  ...
  chassis.waitUntilDone();
  chassis.setPose(74.0 - findDistToWall(0), 74 - findDistToWall(3),
                  currentPose.theta);
  ...
  /* hard corner rings
    ...
    chassis.moveToPoint(48, 62, 2000);
      chassis.waitUntilDone();
      chassis.moveToPoint(48, 24, 2000, {.forwards = false, .maxSpeed =
    max_speed}); chassis.waitUntilDone(); chassis.turnToHeading(15, 2000,
    {.maxSpeed = max_speed1}); chassis.waitUntilDone(); pros::delay(50);
  ...
  */
  ...
  currentPose = chassis.getPose();
  chassis.setPose(72.0 - findDistToWall(1),
                  72.0 - ((distance_bl.get() > distance_br.get())
                              ? (distance_bl.get() / 25.4 + 7.25)
                              : (distance_br.get() / 25.4 + 7.25)),
                  findHeading(1, 180));
  pros::delay(250);
  printPosition((char *)"after reset", true, true);
  ...
  pros::delay(1000);
  // chassis.moveToPoint(56, 0, 1000, {.forwards = false, .maxSpeed
  // =max_speed});
  chassis.moveToPoint(48, 0, 1000, {.forwards = false, .maxSpeed = max_speed});
  chassis.turnToHeading(90, 1000, {.maxSpeed = max_speed1});
  chassis.waitUntilDone();
