...
void redNegativeHalfWP(){//EVERYTHING DONE
  ...
  liftPneumaticUp();
  pros::delay(300);
  chassis.moveToPoint(-7,-60,2000,{.forwards=false,.maxSpeed = 127,.minSpeed = 30, .earlyExitRange = 6});
  chassis.moveToPoint(-19,-26,2000,{.forwards=false,.maxSpeed = 127,.minSpeed = 60, .earlyExitRange = 6},false);
  liftPneumaticDown();
  chassis.moveToPoint(-25,-26,2000,{.forwards=false,.maxSpeed = 100,.minSpeed = 30, .earlyExitRange = 6},false);
  pros::delay(200);
  closeClamp();
  pros::delay(200);
  spinIntake(127);
  ...
  chassis.moveToPoint(-43,-24,1000,{.maxSpeed = 127,.minSpeed = 10, .earlyExitRange = 6});
  startSorting();

  //FIRST RING
  chassis.turnToPoint(-43,-10,1000,{.maxSpeed = 127,.minSpeed = 20, .earlyExitRange = 1});
  chassis.moveToPoint(-43,-10.25,1000,{.maxSpeed = 127,.minSpeed = 30, .earlyExitRange = 6});
  ...
  chassis.moveToPoint(-48,-11.75,1000,{.maxSpeed = 127, .minSpeed = 12, .earlyExitRange = 2},false);
  pros::delay(300);
  ...
  chassis.moveToPoint(-64,-65,600,{.maxSpeed = 127, .minSpeed = 20, .earlyExitRange = 5});
  chassis.moveToPoint(-72,-74,600,{.maxSpeed = 127-30});
  chassis.moveToPoint(-54,-50,1000,{.forwards=false,.maxSpeed = 127,.minSpeed = 20, .earlyExitRange = 8},false);
  chassis.turnToPoint(0,-48,2000,{.maxSpeed = 127,.minSpeed = 20, .earlyExitRange = 8});
  chassis.waitUntil(24);
  liftIntake();
  chassis.moveToPoint(-9,-51,1400,{.maxSpeed = 90});
  chassis.waitUntil(10000);
  dropIntake(); 
  pros::delay(100);
  chassis.moveToPoint(-24,-48,1000,{.forwards=false,.maxSpeed = 127,.minSpeed = 30, .earlyExitRange = 2});
  chassis.turnToPoint(-15,-23,2000,{.maxSpeed = 127,.minSpeed = 20, .earlyExitRange = 8});
  chassis.moveToPoint(-15,-23,1000,{.maxSpeed = 127,.minSpeed = 30, .earlyExitRange = 2});
  chassis.turnToHeading(45, 1000,{},false);
  master.clear_line(0);
  //*/
  int temp = pros::millis();
...
void auton_60s_skills_1() {
  ...
  */
  ...
  hoodFwd();
  redirectRings();
  chassis.turnToPoint(0, 48, 1000, {.forwards = false, .maxSpeed = (int)max_speed, .minSpeed = 30, .earlyExitRange = 3});
  ...
  chassis.turnToHeading(0, 1000, {.maxSpeed = (int)max_speed, .earlyExitRange = 0.5},false);
  closeRedirect();
  currentPose = chassis.getPose();
  pros::delay(50);
  ...
  chassis.turnToPoint(-22, 30, 1000, {.maxSpeed = (int) speed,.minSpeed = 40, .earlyExitRange = 6});
  startSorting();
  chassis.moveToPoint(-22, 30, 3000, {.maxSpeed = speed,.minSpeed = 30,.earlyExitRange=3});
  chassis.waitUntil(6);
  spinIntake(127);
  ...
  chassis.turnToPoint(-3, 3, 3000, {.maxSpeed = (int) speed, .minSpeed = 50,.earlyExitRange = 6});
  pros::delay(100);
  chassis.moveToPoint(-3, 3, 2000, {.maxSpeed = speed, .minSpeed = 50,.earlyExitRange=4}, false);
  printPositionV2((char *)"Middle Ring", false, false, time);

  chassis.turnToPoint(34, 48, 900, {.maxSpeed = (int) speed},false);
  pros::delay(300);
  chassis.moveToPoint(34, 48, 2000, {.maxSpeed = speed, .minSpeed = 50, .earlyExitRange=2});
  startSorting();
  chassis.turnToPoint(50, 48, 1000,{.maxSpeed = (int) speed, .minSpeed = 50,.earlyExitRange = 6});
  hoodBwd();
  chassis.moveToPoint(54, 48, 1000,{.maxSpeed = speed, .minSpeed = 50, .earlyExitRange=2});
  chassis.turnToPoint(72, 72, 1000,{.forwards=false,.maxSpeed = (int) speed, .minSpeed = 12,.earlyExitRange = 3},false);
  pros::delay(50);
  openClamp();
  pros::delay(100);
  driveFor(speed-20, 100);
  closeClamp();
  driveFor(-speed+20, 500);
  printPositionV2((char *) "Drop #3 Goal",false,false,time);

  spinIntake(-127);  
  driveFor(speed-20, 450);
  master.clear();
  ...
  chassis.moveToPoint(10, 48, 2000,{.forwards = false,.minSpeed = 50, .earlyExitRange=6});  
  closeClamp();
  //chassis.turnToPoint(-72, 72,1000,{.forwards=false});
  //chassis.moveToPoint(-15, 60, 1000,{.forwards = false,.minSpeed = 40, .earlyExitRange=3});  

  chassis.moveToPoint(-72, 76,1700,{.forwards=false}, false);
  printPositionV2((char *) "Drop #4 Goal",false,false,time);
  pros::Task lift_for_climb([=] {moveLiftToPos(105);});
  chassis.moveToPoint(-12, 12, 3000, {.maxSpeed = speed, .minSpeed = 60, .earlyExitRange=0.5});
  chassis.moveToPoint(0, 0, 2500,{.maxSpeed = 40});
